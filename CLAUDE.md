...
6. **Google Docs 連結處理規範 (Google Docs Link Handling)**：
   * 當使用者提供 Google Docs 連結（包含 `docs.google.com/document/`）並要求讀取、分析或新增為新課程時，AI **必須使用本機的 `course/import_gdoc.py` 腳本（或其相同的匯出邏輯）來獲取清潔的 Markdown 內文**，嚴禁使用外部網頁讀取工具或直接爬取，以確保每次獲取之內文格式、一級標題、段落與引註表現完全一致。
   * 範例指令：`python course/import_gdoc.py "https://docs.google.com/document/d/[DOC_ID]/edit"`（建議使用 Windows 系統上的全域 Python 3.13 執行，以確保套件完整性）。
