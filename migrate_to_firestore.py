...
def migrate_categories(db, catalog_data):
    ...
    for idx, cat in enumerate(categories):
        ...
        print(f"   ✅ {cat['title']} ({doc_id}) — {len(cat.get('pages', []))} 篇文章")

    print(f"   📊 共上傳 {len(categories)} 個分類")
    ...
    for art in articles:
        ...
        if os.path.exists(article_json_path):
            try:
                ...
                print(f"   ⚠️ 讀取 {article_json_path} 失敗: {e}")
                doc_data['content_html'] = ''
        else:
            print(f"   ⚠️ 找不到 {article_json_path}")
            doc_data['content_html'] = ''
        ...
        try:
            ...
            print(f"   ✅ [{page_id}] {art['title']} ({content_size/1024:.1f} KB)")
            success_count += 1
        except Exception as e:
            print(f"   ❌ [{page_id}] 上傳失敗: {e}")
            error_count += 1
...
def migrate_worklog(db, catalog_data):
    ...
    if worklog_html:
        ...
        print(f"   ✅ 更新日誌已上傳 ({len(worklog_html)/1024:.1f} KB)")
    else:
        print("   ⚠️ 沒有找到更新日誌")
...
def migrate_search_index(db):
    ...
    if not os.path.exists(search_index_path):
        print(f"   ⚠️ 找不到 {search_index_path}")
        return
    ...
    for page_id, blocks in grouped.items():
        try:
            ...
            print(f"   ✅ [{page_id}] {len(blocks)} 個搜尋段落")
        except Exception as e:
            print(f"   ❌ [{page_id}] 搜尋索引上傳失敗: {e}")

    print(f"   📊 共上傳 {len(grouped)} 個頁面的搜尋索引 (總計 {len(search_entries)} 段落)")
...
def main():
    ...
    print(f"\n📊 本地資料: {articles_count} 篇文章, {categories_count} 個分類")
